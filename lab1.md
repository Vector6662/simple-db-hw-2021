...
are [big-endian](http://en.wikipedia.org/wiki/Endianness).（解释位图，然后说明位图最后一个字节的后边的位可能是空闲的，毕竟计算的时候是向上取整）
...
algebra. In SimpleDB, operators are iterator based; each operator implements the `DbIterator` interface.（执行计划？）
