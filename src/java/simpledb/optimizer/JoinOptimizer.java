...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // nested-loops join.
            return cost1 + card1 * cost2 + card1 * card2; //implement 2.2.2
        }
    }
        ...
        for (int len=1; len<=joins.size(); len++) {
            ...
            for (Set<LogicalJoinNode> set : sets) {
                ...
                for (LogicalJoinNode logicalJoinNode : set) {
                    ...
                    CostCard plan = computeCostAndCardOfSubplan(stats, filterSelectivities, logicalJoinNode, set, bestPlan.cost, planCache);
